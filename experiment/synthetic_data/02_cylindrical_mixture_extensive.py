"""Run extensive cylindrical-family grid searches and export aggregated config results.
...
sys.argv = [
    "02_cylindrical_mixture_extensive.py",
    "--output-csv", "results/synthetic_data/cylindrical_config_results.csv",
    "--n-samples", "12000",
...
runpy.run_path("experiment/synthetic_data/02_cylindrical_mixture_extensive.py", run_name="__main__");
"""
    ...
    *,
    ...
    cyl_components = [
        spr.Cylindrical(
            ...
            cross_cov=np.array([[0.45, -0.10, 0.20], [0.05, 0.30, -0.15], [-0.25, 0.10, 0.35]]),
            cond_cov=np.array([[2.20, 0.75, 0.40], [0.75, 0.55, 0.10], [0.40, 0.10, 0.25]]),
            mu_vmf=_unit(np.array([1.0, 0.0, 0.0])),
            kappa=9.0,
        ),
        spr.Cylindrical(
            d_gauss,
            d_vmf,
            mu_gauss=np.array([0.0, 0.0, 0.0]),
            cross_cov=np.array([[-0.30, 0.20, 0.10], [0.15, -0.25, 0.05], [0.10, 0.05, -0.20]]),
            cond_cov=np.array([[0.75, 0.05, 0.00], [0.05, 0.85, 0.08], [0.00, 0.08, 0.70]]),
            mu_vmf=_unit(np.array([0.0, 1.0, 0.0])),
            kappa=9.0,
        ),
        spr.Cylindrical(
            d_gauss,
            ...
            cross_cov=np.array([[0.10, 0.15, -0.20], [-0.05, 0.25, 0.18], [0.20, -0.10, 0.12]]),
            cond_cov=np.array([[0.95, 0.18, -0.05], [0.18, 1.05, 0.12], [-0.05, 0.12, 0.80]]),
            mu_vmf=_unit(np.array([0.0, 0.0, 1.0])),
            kappa=5.5,
        ),
    ]

    generator = spr.MixtureModel(
        components=cyl_components,
        weights=np.array([0.35, 0.35, 0.30], dtype=float),
        init="k-means",
        rng=rng,
    )
...
def _run_searches(data: dict[str, np.ndarray], args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for offset, spec in enumerate(specs):
        seed = int(args.search_seed # + offset
                   )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Sample synthetic cylindrical data, run extensive grid searches for multiple "
            "model families, and export all aggregated config_results to CSV."
        )
    )
    parser.add_argument("--output-csv", type=Path, default=Path("results/synthetic_data/cylindrical_config_results.csv"))
