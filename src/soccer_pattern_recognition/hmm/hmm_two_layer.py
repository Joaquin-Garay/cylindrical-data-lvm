...
from .two_layer_emission_io import TwoLayerEmissionIOMixin
...
class TwoLayerEmission(TwoLayerEmissionIOMixin, BaseEmission):
    """
        ...
        for state in range(n_states):
            ...
            for action in self.model_actions:
                ...
                layer1_mixture = MixtureModel(
                                [MultivariateGaussian(2) for _ in range(n_comp_layer1)],
                                init=value[1],
                                rng=random_state,
                                )
                initialize_model(
...
class TwoLayerHMM(EmissionHMM):
    ...
    def print_params(self, *, decimals: int = 3, show_mom: bool = True) -> None:
